...
Get analytics events for a user session.

**Endpoint:** `GET /user_sessions/{session_id}/analytics`

**Path Parameters:**

- `session_id` (uuid, required): User session UUID

**Query Parameters:**

- `limit` (integer, optional): Events to return, max 100 (default: 20)
- `offset` (integer, optional): Events to skip (default: 0)
...
**Endpoint:** `POST /user_sessions/{session_id}/analytics`
...
```json
{ "event_type": "page_view", "properties": { "page": "/chat" } }
```

**Parameters:**

- `event_type` (string, required): Event type identifier (max 128 chars)
- `properties` (object, optional): Arbitrary event metadata

**Response:** `201 Created` with the created analytics event object
...
**Endpoint:** `GET /analytics/{analytics_id}`

**Path Parameters:**

- `analytics_id` (uuid, required): Analytics event UUID

**Response:** `200 OK` with analytics event object

---

## Update Analytics Event

Update a specific analytics event.

**Endpoint:** `PUT /analytics/{analytics_id}`
...
```json
{ "event_type": "updated_event", "properties": {} }
```

**Response:** `200 OK` with updated analytics event object
...
**Endpoint:** `DELETE /analytics/{analytics_id}`
